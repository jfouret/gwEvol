...
Model optimisation were performed from alignements and tree mentioned above using PAML (Phylogenetic Analysis by Maximum Likelihood) driven by ete3. Then models were compared using the loglikelihood ratio test (LRT) using the chi2 distribution thus if the likelihood of the alternative model is sufficiently superior to the one of the null model the alternative model is supposed to significantly fit better the data than the null model. The table below summerized meanings of LRT tests. Two methods were used to detect positive selection: branch and branch-site methods. A gene is under branch positive selection if the LRT test b_free vs M0 is significant and is in model b_free the estimated dN/dS on foreground branch is superior to 1. A gene is under branch-site positive selection if both LRT test bsA vs M1a and bsA vs bsA1 are significants, in addition for every sites the probability of the dN/dS to be superior to 1 is calculated using Bayes Empirical Bayes method.
...
* Extracting parameters and likelihood from the best fit for each model
* NOT FINISHED
